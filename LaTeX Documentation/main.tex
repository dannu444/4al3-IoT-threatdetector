...
After applying these hyperparameters and training the model, the following results were concluded, where accuracy is determined based on the final FS score:

accuracy on validation data:   0.9453 \\
accuracy on baseline model:    0.8956 \\
accuracy on testing data:      0.9477

The graphs generated for the above results can be found in Figures ~\ref{fig:experiments} through ~\ref{fig:experiments} in the appendix.
...
When comparing validation and testing accuracy, the difference between the two is less than 1\%. This suggests that the model is generalizing well, as there are no signs of overfitting the data.

Overall, both validation and testing show improvement over the baseline accuracy by approximately 5\%. This indicates that the hyperparameter tuning helps improve the model. However, the high baseline value may indicate dataset issues that should be further explored to rule out. Some of these issues may include class imbalance, low variability among samples, or an insufficiently diverse data distribution. This will likely be further explored in future development. 
