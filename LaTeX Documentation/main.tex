...
\title{Group 29 Final Report:\\IoT Attack Detection Machine Learning Model}
...
Upon initial overall assessment, the model does fairly well across all 4 models that the data was testing. Consistently, models return the highest overall accuracy and f-scores for models that were trained on either the dataset with no modifications implemented or the data that implemented sampling. When analyzing the results, it is worth noting the drops in performance across all models when the dataset integrated downsampling. Given that performance remains high both with the original dataset and after removing correlated features, this suggests that these models may still be overfitting to the dominant classes. At the same time, the decreased performance from downsampling means that the model is likely sensitive to the loss of integrity within the dataset. If continuing to work on this model, this issue may have to be addressed by further tailoring the dataset to ensure it prioritizes the representation of data that is classified in the minority without losing the integrity of the data. Even just looking at the diagonal row of the confusion matrix for upsampling demonstrated the promising, high results that are produced. 

While overall accuracy and f-scores were derived to assess effectiveness of the model overall, error within our model was systematically broken down through confusion matrices and per-class data analysis that has been summarized in Table~\ref{tab:table2}, Table~\ref{tab:table3}, Table~\ref{tab:table4}, and Table~\ref{tab:table5}. Evaluating confusion matrices as well as the numerical values of accuracy, precision, and recall for each class, allowed us to accurately assess how our model was handling each type of identification and to determine what areas of identification our model struggled with the most. When looking at these values, classes 3, 4, 7, 8, 9 and 10 all reported extremely high accuracy, precision and recall across all models. This shows that they were the easier classes to recognize, even on models that may not be as optimal for this dataset. Classes such as 1, 2 and 11 did have very high results in certain models. However, this was not consistent throughout all models. Interestingly enough, those 3 classes only reported at high, consistent levels for the decision tree model and neural network. What is ever more notable, especially with the neural network and decision tree, is that the performance across classes 2, 5, 6, and 12 show significant improvements with the upsampling data. Relating back to what was said prior, this could be an indicator that those features are very sensitive to loss in data integrity.  To address this issue, further upsampling, data augmentation, and regularization should be considered in the training implementation to reduce dependency on certain samples within the data. 

One final consideration to keep in mind when further addressing issues in the code is that feature engineering can completely compromise the ability to identify certain classes, notably class 6. Despite the correlation threshold being 90\%, this is still the case. Even ignoring this loss of class data, the classifiers that implement feature selection show little to no improvement across all models. If further work is done to experiment with feature removal, the relationship between classes and each feature should be carefully reviewed in the decision to remove a feature since feature comparison has proved inadequate alone.
...
Key Contribution Area: Testing, Hyperparameter Tuning \& Metric Evaluation
...
\begin{figure*}[h]
  ...
  \caption{
    Confusion matrices for the SVC (left) and neural network (right) baselines when data was reduced to 10,000 instances of downsampling.
  }
  \label{fig:figure15}
\end{figure*}
\vspace{-4pt}
...
\begin{table*}
  \centering
  \caption{Removed Features Based on 0.9 Absolute Correlation Threshold}
  \label{tab:table7}
  \begin{tabular}{|l|p{12cm}|}
    \hline
    fwd\_pkts\_tot\\
    \hline
    bwd\_data\_pkts\_tot\\
    \hline
    fwd\_pkts\_per\_sec\\
    \hline
    flow\_pkts\_per\_sec\\
    \hline
    fwd\_header\_size\_min\\
    \hline
    bwd\_header\_size\_tot\\
    \hline
    bwd\_header\_size\_min\\
    \hline
    flow\_ACK\_flag\_count\\
    \hline
    fwd\_pkts\_payload.std\\
    \hline
    bwd\_pkts\_payload.tot\\
    \hline
    bwd\_pkts\_payload.std\\
    \hline
    flow\_pkts\_payload.max\\
    \hline
    flow\_pkts\_payload.tot\\
    \hline
    fwd\_iat.max\\
    \hline
    fwd\_iat.tot\\
